...
// Ensure the user has accepted the privacy contract before invoking chooseMedia.
// From base library 2.32.3 the platform blocks undeclared privacy interfaces; the
// first run shows a guiding dialog instead of failing silently.
    ...
    wx.getPrivacySetting({
      success(res) {
        ...
        wx.showModal({
          ...
          content: 'Savor needs access to your album to keep up to four photos. Your photos are stored only on this device.',
      ...
      fail() { resolve(); }, // an unknown privacy state must not block the picker
    });
  });
}

function choosePhotos(count) {
  return ensurePrivacy().then((allowed) => new Promise((resolve, reject) => {
    if (allowed === false) { resolve([]); return; }
    wx.chooseMedia({
