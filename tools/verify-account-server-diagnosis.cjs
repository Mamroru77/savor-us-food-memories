...
async function main(){
  ...
  {
    let reads=0;const run=make({find:async()=>{reads++;},insert:async()=>{}},{context:()=>({OPENID:'owner-secret',APPID:1n})});await assert.rejects(run.handler({action:'bootstrap',protocolVersion:1}),TypeError);const res={};
    assert.equal(reads,0);assert.equal(run.logs[0].stage,'account-build');safe(run,res);
  }
  {
