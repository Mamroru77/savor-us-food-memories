...
(async()=>{
  ...
  await test('current native avatar result survives refresh',async()=>{const h=profileEditor();enter(h);const q=avatar(h);q.resolve(['chosen']);await flush();assert.equal(h.p.data.profileAvatar,'chosen');h.p.refresh();assert.equal(h.p.data.profileAvatar,'chosen');});
  ...
  await test('cancel leaves edited fields intact',async()=>{const h=profileEditor();enter(h);edit(h,'onProfileName','draft');const q=avatar(h);q.reject({errMsg:'chooseMedia:fail cancel'});await flush();assert.equal(h.p.data.profileName,'draft');assert.equal(h.state.profile.avatar,'/images/jamie.jpg');});
  await test('newer chooser wins even if old chooser completes last',async()=>{const h=profileEditor();enter(h);const a=avatar(h),b=avatar(h);b.resolve(['new']);await flush();a.resolve(['old']);await flush();assert.equal(h.p.data.profileAvatar,'new');});
  await test('avatar chooser survives app re-verification for the same owner',async()=>{const h=profileEditor();enter(h);edit(h,'onProfileName','draft');const q=avatar(h);assert(h.events.some(e=>e.name==='nativeavatar'&&e.detail.phase==='start'));h.state.identity={userId:'',generation:2,locked:true,status:'verifying'};h.emit();hide(h);assert.equal(h.p.data.profileName,'draft');h.state.identity={userId:'fixture',generation:2,locked:false,status:'verified'};h.emit();enter(h);q.resolve(['chosen-after-resume']);await flush();assert.equal(h.p.data.profileAvatar,'chosen-after-resume');assert.equal(h.p.data.profileName,'draft');});
  ...
  await test('refresh cannot enable Save while a chooser is pending',async()=>{const h=profileEditor();enter(h);const q=avatar(h);h.p.refresh();assert.equal(h.p.data.profileUploading,true);h.p.onProfileSave();assert(!h.events.some(e=>e.name==='close'));q.resolve(['ready']);await flush();});
  ...
  await test('switch away and back invalidates old chooser',async()=>{const h=profileEditor();enter(h);const q=avatar(h);leave(h);enter(h);q.resolve(['old']);await flush();assert.notEqual(h.p.data.profileAvatar,'old');});
