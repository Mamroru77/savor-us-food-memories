...
section('P. tab selected mapping');
{
  ...
  for (const [pagePath, index] of Object.entries(expected)) {
    ...
    const ok = /onShow\s*\(/.test(js) && /getTabBar\s*&&\s*this\.getTabBar\s*\(\)/.test(js)
      && new RegExp(`tabBar\\.showSelection\\(\\s*${index}\\s*,`).test(js) && /tabBar\.showSelection\(/.test(js);
...
function loadDataInSandbox(sandbox) {
  return loadUIUtilityInSandbox(sandbox, 'data');
}
...
function loadUIUtilityInSandbox(sandbox, name) {
  if (!['data', 'i18n', 'locales', 'pageHeadings', 'localDate', 'profileRepository', 'settingsRepository', 'memoryRepository'].includes(name)) throw new Error('unexpected UI dependency ' + name);
