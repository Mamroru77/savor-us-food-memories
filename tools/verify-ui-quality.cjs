...
test('Save uses loader for both busy branches, idle NotebookPen, and disabled semantics',()=>{
 assert(addWxml.includes('wx:if="{{uploading || saving}}" name="loader-circle"'));assert(addWxml.includes('wx:else name="notebook-pen"'));assert(!addWxml.includes('wx:elif="{{saving}}"'));assert(addWxml.includes('aria-disabled="{{uploading || saving}}"'));assert(appCss.includes('.quiet .spin'));
...
test('Memory Row style is held unchanged pending native divider reproduction',()=>{
 const hash=require('node:crypto').createHash('sha256').update(read('components/memory-row/index.wxss')).digest('hex');assert.equal(hash,'496630458c2b161f0227d5eae9a12b0ad6624884a67109ff3215e122b72b9976');
});
