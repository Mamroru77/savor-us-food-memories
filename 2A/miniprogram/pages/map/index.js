// Map First: native <map> with photo pins rendered as always-on custom callouts, search, filters and a place card.
const store = require('../../utils/store');
const { photos, formatDate, plural } = require('../../utils/data');
...
Page({
  data: {
    ...
    markers: [], visible: 0, visibleLabel: '', visibleList: [], selected: null, selectedId: 'comptoir', mapFailed: false,
    stageBottom: 0,
  },

  onLoad() {
    const layout = getApp().getLayout();
    this.setData({ stageBottom: layout.safeBottom });
  },
  ...
  onHide() { this.release(); },
  onUnload() { this.release(); if (this.fitTimer) clearTimeout(this.fitTimer); },
  ...
  compute(memories, query, filter, selectedId) {
    ...
    return {
      ...
      visibleLabel: plural(visible.length, 'memory', 'memories') + ' found',
      visibleList: visible,
      markers,
      selected: place,
  ...
  fit(computed) {
    if (!this.mapContext || this.data.mapFailed || !computed.markers.length) return;
    ...
    this.mapContext.includePoints({ points, padding: [110, 40, 220, 40] });
  },
  scheduleFit(computed) {
    if (this.fitTimer) clearTimeout(this.fitTimer);
    this.fitTimer = setTimeout(() => { this.fitTimer = null; this.fit(computed); }, 320);
  },

  onQuery(event) { this.scheduleFit(this.refresh({ query: event.detail.value, showFilters: false })); },
  ...
  recenter() {
    ...
    if (!this.mapContext || this.data.mapFailed) return;
    this.mapContext.includePoints({
      points: [
        { latitude: PARIS.latitude + 0.012, longitude: PARIS.longitude - 0.02 },
        { latitude: PARIS.latitude - 0.012, longitude: PARIS.longitude + 0.02 },
      ],
      padding: [80, 20, 200, 20],
    });
  },
  onPinTap(event) {
  ...
  onMapError() { this.setData({ mapFailed: true, showFilters: false }); },
  selectFromList(event) { this.refresh({ selectedId: event.detail.id }); },
  openPlace() {
  ...
  toggleSaved() {
    ...
    store.toast(selected.saved ? 'Place removed from your favorites.' : 'A good place to come back to. Saved.');
  },
});
