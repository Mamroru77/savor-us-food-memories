// Add a Memory: photos, place, notes, date, rating, tags. The draft survives leaving the tab and relaunching.
const store = require('../../utils/store');
const image = require('../../utils/image');
const { photos, createId, isValidDate, isSafeImage, today, cityLocations, formatDate, DRAFT_KEY } = require('../../utils/data');
...
function loadDraft() {
  try {
    ...
    if (saved && typeof saved === 'object' && typeof saved.restaurant === 'string' && typeof saved.notes === 'string'
      ...
      return Object.assign({}, starter, saved, { photos: saved.photos.map(image.materializePhoto), city: CITIES.indexOf(saved.city) >= 0 ? saved.city : 'Paris' });
    }
...
Page({
  data: {
    theme: 'pearl', dusk: false, quiet: false, ink: '#1b1c1a', muted: '#565752', buttonInk: '#ffffff',
    draft: starter, dateLabel: '', cities: CITIES, cityIndex: 0, knownPlace: false, stars: [], placeSuggestions: [],
    showTagInput: false, tag: '', suggestions: [], error: '', busy: false, saving: false, today: today(), lastPhoto: '',
  },
  ...
  onHide() { this.release(); this.flushDraft(); },
  onUnload() { this.release(); this.flushDraft(); },
  ...
  derive(draft, state) {
    ...
    const names = [];
    state.memories.forEach((item) => { if (names.indexOf(item.restaurant) < 0) names.push(item.restaurant); });
    const placeSuggestions = name && !known && this.focusedRestaurant
      ? names.filter((value) => value.toLowerCase().indexOf(name) >= 0).slice(0, 3) : [];
    return {
      knownPlace: known,
      suggestions,
      placeSuggestions,
      ...
      lastPhoto: draft.photos.length > 1 ? draft.photos[draft.photos.length - 1] : '',
    };
  },

  applyDraft(draft) {
    this.setData(Object.assign({ draft }, this.derive(draft, store.getState())));
    this.scheduleDraftSave(draft);
  },
  patch(changes) { this.applyDraft(Object.assign({}, this.data.draft, changes)); },
  scheduleDraftSave(draft) {
    this.pendingDraft = draft;
    if (this.draftTimer) clearTimeout(this.draftTimer);
    this.draftTimer = setTimeout(() => this.flushDraft(), 250);
  },
  flushDraft() {
    if (this.draftTimer) { clearTimeout(this.draftTimer); this.draftTimer = null; }
    if (!this.pendingDraft) return;
    try { wx.setStorageSync(DRAFT_KEY, this.pendingDraft); } catch (error) { /* the form still works without a saved draft */ }
    this.pendingDraft = null;
  },

  onField(event) { this.patch({ [event.currentTarget.dataset.key]: event.detail.value }); },
  onRestaurantFocus() { this.focusedRestaurant = true; this.applyDraft(this.data.draft); },
  onRestaurantBlur() {
    this.focusedRestaurant = false;
    setTimeout(() => { if (!this.focusedRestaurant) this.setData({ placeSuggestions: [] }); }, 200);
  },
  pickSuggestion(event) { this.focusedRestaurant = false; this.patch({ restaurant: event.currentTarget.dataset.name }); },
  ...
  pick(count, apply) {
    ...
    image.choosePhotos(count)
      ...
      .catch((error) => this.setData({ error: error.message || 'That photo could not be opened. Please try another.' }))
  ...
  replacePhoto() {
    this.pick(1, (paths) => {
      ...
      list[0] = paths[0];
      if (previous && previous !== paths[0] && list.indexOf(previous) < 0) image.removePhoto(previous);
      this.patch({ photos: list.slice(0, 4) });
    });
  },
  addPhotos() {
    const room = 4 - this.data.draft.photos.length;
    if (room <= 0) { store.toast('You can keep up to four photos in one memory.'); return; }
    this.pick(room, (paths) => this.patch({ photos: this.data.draft.photos.concat(paths).slice(0, 4) }));
  },
  removeLastPhoto() {
    const list = this.data.draft.photos.slice();
    const removed = list.pop();
    if (list.indexOf(removed) < 0) image.removePhoto(removed);
    this.patch({ photos: list });
  },
  previewMain() {
    const list = this.data.draft.photos;
    if (list.length) wx.previewImage({ current: list[0], urls: list });
  },
  ...
  save() {
    ...
    if (!isValidDate(draft.date) || draft.date > today()) { this.setData({ error: 'Choose a date for your memory.' }); return; }
    ...
    this.flushDraft();
    store.toast('A little moment, kept forever. Memory saved.', 'success');
    setTimeout(() => {
