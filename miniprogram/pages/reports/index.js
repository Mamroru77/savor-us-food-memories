...
Page({
 ...
 reset(){secondaryUI.sync(this);this.clear();const s=identity.snapshot(),year=new Date().getFullYear(),years=Array.from({length:year-1899},(_,i)=>String(year-i));this.setData({locked:s.locked,years,year:String(year),dusk:store.get().settings.theme==='dusk'});if(!s.locked&&!this._hidden)this.generate();},
 ...
 generate(){
  ...
  this.setData({report:this._report,message:''},()=>{if(!this.alive(token)||serial!==this._renderSerial)return;wx.createSelectorQuery().in(this).select('#report-canvas').fields({node:true,size:true}).exec(result=>{if(!this.alive(token)||serial!==this._renderSerial||!result||!result[0]||!result[0].node)return;const canvas=result[0].node;canvas.width=720;canvas.height=1040;report.draw(canvas.getContext('2d'),this._aggregate);this._canvas=canvas;});});
 },
 async exportHtml(){
