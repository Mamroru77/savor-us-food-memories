...
Page({
  ...
  onShow() {
    ...
    this.readMapScale();
    return store.syncCloud().catch(() => store.notify(i18n.t('地图正在显示缓存记录，云同步暂不可用。')));
  },
  ...
  renderDrawerPhotos(generation) {
    if(!this.pinRenderer||!this.active)return;
    ...
    this.data.mapDrawers.forEach(drawer=>[drawer.root].concat(drawer.rows).forEach(pin=>{
      ...
      renderer.render(memory,pin.selected).then(path=>{
        if(this.disposed||!this.active||generation!==this.markerGeneration||renderer!==this.pinRenderer)return;
        this.setData({mapDrawers:this.data.mapDrawers.map(d=>d.rootId!==drawer.rootId?d:Object.assign({},d,{
          root:d.root.id===pin.id&&d.root.selected===pin.selected?Object.assign({},d.root,{iconPath:path}):d.root,
          rows:d.rows.map(row=>row.id===pin.id&&row.selected===pin.selected?Object.assign({},row,{iconPath:path}):row)
        }))});
      }).catch(()=>{});
    }));
  },
  refreshStackFrame() {
    this.setData({mapDrawers:this.buildDrawers(this.markerGroups||[],this.data.markers,this.data.selectedId)});
  },
  onStackPress(e) {
  ...
  stopDrawerReveal() {if(this.drawerRevealTimer)clearTimeout(this.drawerRevealTimer);this.drawerRevealTimer=null;this.drawerProgress=0;},
  startDrawerReveal(target) {
    const root=this.drawerRootId,from=this.drawerProgress||0,start=Date.now(),duration=mapStack.DURATION;
    const step=()=>{
      this.drawerRevealTimer=null;
      if(!this.active||this.disposed||this.drawerRootId!==root)return;
      const t=Math.min(1,(Date.now()-start)/duration);
      this.drawerProgress=from+(target-from)*mapStack.ease(t);
      this.refreshStackFrame();
      if(t<1)this.drawerRevealTimer=setTimeout(step,20);
    };
    this.drawerRevealTimer=setTimeout(step,20);
  },
  onDrawerPage(e) {
  ...
  renderMarkerPhotos(visible, selectedId, generation) {
    ...
    ordered.forEach(({m,index})=>{
      renderer.render(m,m.id===selectedId).then(path=>{
        if(!this.active || this.disposed || generation!==this.markerGeneration || renderer!==this.pinRenderer) return;
  ...
  onMapRegionChange(e) {
    ...
    this.setData({stackPositionsReady:false});
    if(phase==='begin') {this.stackGesture=true;return;}
    this.stackGesture=false;
    this.readMapScale(e && e.detail && e.detail.scale);
    if(this.active&&!this.disposed) {
      this.stackAlignTimer=setTimeout(()=>{
        this.stackAlignTimer=null;this.syncStackPositions();
      },80);
    }
  },
  readMapScale(value) {
  ...
  onFillLocation() {
    ...
    wx.showActionSheet({ itemList: batch.map(m => (m.restaurant + ' · ' + m.date).slice(0, 48)),
      success: result => this.pickLocationFor(batch[result.tapIndex]) });
  },
  ...
  pickLocationFor(memory) {
    ...
    return locations.choose(memory).then(location => store.setMemoryLocation(memory, location))
      .then(() => store.notify(i18n.t('餐馆位置已保存。')))
      .catch(error => { if (!error.cancelled) store.notify(error.message || i18n.t('位置保存失败，请重试。')); })
