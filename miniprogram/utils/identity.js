...
let diagnosisActive=false,diagnosisAttempted=false,lastDiagnosis=null,diagnosticCalls={account:0,handshake:0};
const listeners=[];
const DIAGNOSTIC_CODES={
  ...
  'partition-accept':['CACHE_CORRUPT','CACHE_UNSUPPORTED','CACHE_MISSING','OUTBOX_IDENTITY_MISMATCH','DRAFT_IDENTITY_MISMATCH','PARTITION_ACCEPT_FAILED','STALE_IDENTITY'],
  complete:['OK']
};
function cache(){ if(!partitions) partitions=createPartitions(wx); return partitions; }
function error(code){const e=new Error('请联网验证当前账号后重试。原有数据与草稿已保留。');e.code=code;return e;}
...
function enableDiagnosis(){diagnosisActive=true;diagnosisAttempted=false;lastDiagnosis=null;diagnosticCalls={account:0,handshake:0};invalidate();}
...
function assertLease(token){if(!current||!token||token.userId!==current.userId||token.generation!==epoch)throw error('STALE_IDENTITY');cache().assertLease(current.partition);}
...
async function verify(options){
  ...
  const task=(async()=>{
    try{
      ...
      current={userId:partition.userId,generation,partition};phase='verified';if(diagnostic)recordDiagnosis('complete','OK',startedAt,handshake,true);emit();return lease();
...
async function resumeNative(token){if(flight)await flight;const next=lease();if(!token||token.userId!==next.userId)throw error('STALE_IDENTITY');return next;}
