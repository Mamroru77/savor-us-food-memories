...
async function call(action,args={},token=identity.lease()){
 identity.assertLease(token);records.initCloud();
...
function directory(token){identity.assertLease(token);const path=wx.env.USER_DATA_PATH+'/savor-workspace/'+token.userId;try{wx.getFileSystemManager().accessSync(path);}catch(e){wx.getFileSystemManager().mkdirSync(path,true);}return path;}
...
async function mutate(action,args){const token=identity.lease();if(identity.workspaceIntent())fail('WORKSPACE_PENDING');const intent={actorUserId:token.userId,operationId:data.createId(),action,args};keep(intent,token);return execute(intent,token);}
async function createArchive(rows,kind,originalRaw){
...
function retry(){const token=identity.lease(),intent=identity.workspaceIntent();if(!intent)fail('NO_PENDING_OPERATION');return execute(intent,token);}
