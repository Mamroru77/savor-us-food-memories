...
function readBase64(filePath,token){
 const root=wx.env.USER_DATA_PATH+'/savor-photos/'+token.userId+'/';
...
async function compressForSharing(filePath){
 ...
 const root=wx.env.USER_DATA_PATH+'/savor-photos/'+token.userId+'/';if(!filePath.startsWith(root)||filePath.includes('..'))fail('INVALID_LOCAL_PHOTO');
...
async function upload(scope,filePath){
...
async function retry(){const token=identity.lease(),intent=identity.mediaIntent();if(!intent)fail('NO_MEDIA_PENDING');return execute(intent,token);}
...
async function read(scope,mealId,displayDeadline){
