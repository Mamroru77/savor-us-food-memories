...
const rows={
 frozenSave:["保存已冻结","Saving paused"],
 frozen:["当前为诊断冻结模式，保存与同步暂不可用。这不是网络错误；原有数据不会删除。","Diagnostic freeze is active. Saving and sync are disabled. This is not a network error; existing data is not deleted."],
 frozenTitle:["诊断模式：保存与同步已冻结","Diagnostic mode: saving and sync paused"],
 frozenGate:["当前版本仅用于诊断。验证成功也不会解除业务冻结；原有数据保留。","This build is for diagnostics. Successful verification does not enable business operations. Existing data is retained."],
 diagnose:["执行一次身份诊断","Run one identity diagnostic"],diagnosed:["本轮诊断已执行","Diagnostic already attempted"],
 draftFailed:["本次输入尚未确认保存到本机，请勿离开页面。检查存储空间后重试；不会主动清除已有数据。","Saving these edits has not been confirmed. Stay on this page, check available storage and retry. Existing data is not intentionally cleared."],
...
module.exports=function(state){const index=i18n.locale()==='zh-CN'?0:1;const labels=Object.fromEntries(Object.keys(rows).map(k=>[k,rows[k][index]]));if(state&&state.diagnosisActive){labels.verify=labels.frozenTitle;labels.gate=labels.frozenGate;labels.retry=state.diagnosisAttempted?labels.diagnosed:labels.diagnose;}return labels;};
