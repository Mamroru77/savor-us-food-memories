...
Page({
  data: {
    ...
    themes: THEMES, bars: BARS, faqs: FAQS, alexPhoto: photos.alex, today: today(), storageNote: '',
  },
  ...
  // Removes photo files no memory, avatar, or draft points at, then reports the result.
  freeUpSpace() {
    const state = store.getState();
    ...
    } catch (error) { /* a missing draft is fine */ }
    let removed = 0;
    try { removed = image.cleanupOrphans(keep.filter(Boolean)); } catch (error) { removed = 0; }
    this.setData({ storageNote: removed ? removed + (removed === 1 ? ' unused photo file removed.' : ' unused photo files removed.') : 'Nothing to clean up. Your photos are all in use.' });
  },
