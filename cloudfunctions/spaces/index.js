...
exports.main=createHandler({context:()=>cloud.getWXContext(),repository:{
 transaction:fn=>db.runTransaction(async transaction=>fn({
  async get(collection,id){
   ...
   try{const result=await transaction.collection(collection).doc(id).get();return result.data||null;}
   catch(e){if(e.errCode===-502004||/DOCUMENT_NOT_FOUND|document.*(?:not exist|not found)|文档不存在/i.test(String(e.errMsg||e.message)))return null;throw e;}
  },
