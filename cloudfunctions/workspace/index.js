...
cloud.init({env:cloud.DYNAMIC_CURRENT_ENV});const db=cloud.database(),env=process.env;
...
const config={templateId:env.REMINDER_TEMPLATE_ID||'',thingKey:env.REMINDER_THING_KEY||'',timeKey:env.REMINDER_TIME_KEY||'',feedbackReady:!!(endpoint&&env.FEEDBACK_WEBHOOK_TOKEN),feedbackIdempotent:env.FEEDBACK_IDEMPOTENT==='true',workerSecret:env.WORKSPACE_WORKER_SECRET||'',adminOpenids:(env.WORKSPACE_ADMIN_OPENIDS||'').split(',').map(s=>s.trim()).filter(Boolean)};
const repository={
 transaction:fn=>db.runTransaction(async tx=>fn({
  async get(c,id){try{return (await tx.collection(c).doc(id).get()).data||null;}catch(e){if(e.errCode===-502004||/DOCUMENT_NOT_FOUND|document.*(?:not exist|not found)|文档不存在/i.test(String(e.errMsg||e.message)))return null;throw e;}},
