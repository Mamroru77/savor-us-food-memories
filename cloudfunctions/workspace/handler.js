...
const jobView=j=>({id:j._id,type:j.type,state:j.state,deliveryMode:j.deliveryMode||'legacy',dueAt:j.dueAt,dueText:j.type==='reminder'?new Date(j.dueAt-j.timezoneOffset*60000).toISOString().slice(0,16).replace('T',' ')+' (UTC'+(j.timezoneOffset<=0?'+':'-')+Math.abs(j.timezoneOffset/60)+')':'',title:j.title||'',message:j.type==='feedback'?j.message:'',code:j.code||'',attempts:j.attempts||0,createdAt:j.createdAt,acceptedAt:j.acceptedAt||null});
...
function createHandler({context,repository,transport,config={},now=Date.now,random=()=>crypto.randomBytes(16).toString('hex')}){
 const cloudOnly=()=>config.feedbackMode==='cloud_only';
 const configured=type=>type==='reminder'?!!(!config.remindersDisabled&&/^[A-Za-z0-9_-]{10,200}$/.test(config.templateId||'')&&/^thing\d+$/.test(config.thingKey||'')&&/^time\d+$/.test(config.timeKey||'')&&transport&&transport.reminder):!!(!cloudOnly()&&config.feedbackReady&&transport&&transport.feedback);
 ...
 async function worker(event,wx){
  ...
  for(const candidate of candidates){
   ...
   const claimed=await repository.transaction(async tx=>{
    ...
    // Storage-only tickets never become deliveries, even after future configuration changes.
     if((j.type==='feedback'&&(cloudOnly()||j.deliveryMode!=='external'||j.state==='stored'))||(j.type==='reminder'&&config.remindersDisabled))return null;
     if(j.state==='sending'){
 ...
 return async(event={})=>{
  try{
   ...
   const result=await repository.transaction(async tx=>{
    ...
    if(event.action==='config'){const space=await activeSpace();return ok({templateId:config.templateId||'',reminderReady:configured('reminder'),feedbackReady:configured('feedback'),feedbackStorageReady:true,feedbackMode:cloudOnly()?'cloud_only':'external',remindersDisabled:!!config.remindersDisabled,space:space?{id:space._id,version:space.version}:null});}
    ...
    if(event.action==='submitFeedback'||event.action==='scheduleReminder'){
     ...
     const fingerprint=digest(body),old=await tx.get(C.jobs,id);if(old){if(!reminder&&event.deliveryMode==='cloud_only'&&old.deliveryMode!=='cloud_only')fail('OPERATION_CONFLICT');if(old.fingerprint!==fingerprint)fail('OPERATION_CONFLICT');return ok({job:jobView(old)});}
     if(reminder){
     ...
     const type=reminder?'reminder':'feedback',ready=configured(type),storageOnly=!reminder&&(cloudOnly()||event.deliveryMode==='cloud_only');
     const j={_id:id,ownerUserId:uid,toOpenid:wx.OPENID,type,...body,fingerprint,deliveryMode:storageOnly?'cloud_only':'external',state:storageOnly?'stored':ready?'queued':'blocked_config',code:storageOnly?'STORED_ONLY':ready?'':'DELIVERY_NOT_CONFIGURED',dueAt:reminder?body.dueAt:time,nextAttemptAt:reminder?body.dueAt:time,createdAt:time,attempts:0};
    ...
    if(['jobStatus','cancelJob','retryJob'].includes(event.action)){
     ...
     if(event.action==='cancelJob'){
      ...
      if(event.consent!==true)fail('CONSENT_REQUIRED');
      if(cloudOnly()||j.deliveryMode!=='external'||j.state==='stored')fail('RETRY_UNSAFE');
